...
|---------|---------------|-------|
| **Ports** | 5× Gigabit Ethernet + 1× SFP | WAN, VLANs, management |
...
- **eth2**: VLAN77 trunk (to Zyxel switch → blades 001-004)
- **eth3**: VLAN77 access (direct to blade005)
- **eth4**: VLAN88 (home network)
- **eth5**: VLAN100 (management workstation, 1Gbps)
...
**Note:** blade005 connected directly to MikroTik eth3 (separate VLAN access port)
...
│  ┌──────────────────────────────────────────────────────┐   │
│  │  Firewall │ VLAN Routing │ DNS │ DHCP │ WireGuard    │   │
│  └──────────────────────────────────────────────────────┘   │
├────────────┬──────────────┬──────────────┬──────────────────┤
│ VLAN77     │  VLAN88      │  VLAN100     │  WireGuard VPN   │
│ (Cluster)  │  (Home)      │  (Mgmt)      │  (Remote)        │
...
| VLAN | Network | Gateway | DNS | Purpose | Firewall Policy |
|------|---------|---------|-----|---------|----------------|
| **77** | 192.168.77.0/24 | .77.1 | .77.1 | K3s cluster nodes | Allow internet, block VLAN88→77 except 80/443, block VLAN100 |
| **88** | 192.168.88.0/24 | .88.1 | .88.1 | Home devices | Allow internet, allow 77:80/443 (web services) |
| **100** | 192.168.100.0/24 | .100.1 | .100.1 | Admin workstation | Full access (all VLANs + internet), no inbound |
| **WG** | 10.8.0.0/24 | .77.15 | .77.1 | VPN clients | Access VLAN77 + pod/service networks, block VLAN88/100 |
