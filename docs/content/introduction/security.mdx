...
**Design Rationale**: Isolate trust zones to limit blast radius

| VLAN | Purpose | Trust Level | Access Policy |
|------|---------|-------------|---------------|
| **VLAN77** (Cluster) | K3s nodes | Medium | Internet yes, VLAN88 no, VLAN100 no |
| **VLAN88** (Home) | Personal devices | Low | Internet yes, VLAN77:80/443 only |
| **VLAN100** (Management) | Admin workstation | High | All networks (no inbound) |
| **WireGuard VPN** | Remote access | Medium | VLAN77 + pods, no VLAN88/100 |

### Firewall Rules (MikroTik)

**Default Policy**: Deny all, explicit allow

**VLAN77 (Cluster) Rules:**
```routeros
# Allow established/related connections
/ip firewall filter add chain=forward connection-state=established,related action=accept

# Allow VLAN100 (management) to VLAN77 (cluster)
/ip firewall filter add chain=forward src-address=192.168.100.0/24 dst-address=192.168.77.0/24 action=accept

# Allow VLAN88 (home) to VLAN77 ports 80,443 only (web services)
/ip firewall filter add chain=forward src-address=192.168.88.0/24 dst-address=192.168.77.0/24 protocol=tcp dst-port=80,443 action=accept

# Allow WireGuard VPN to VLAN77
/ip firewall filter add chain=forward src-address=10.8.0.0/24 dst-address=192.168.77.0/24 action=accept

# Allow WireGuard VPN to pod network (10.42.0.0/16)
/ip firewall filter add chain=forward src-address=10.8.0.0/24 dst-address=10.42.0.0/16 action=accept

# Allow WireGuard VPN to service network (10.43.0.0/16)
/ip firewall filter add chain=forward src-address=10.8.0.0/24 dst-address=10.43.0.0/16 action=accept
...
/ip firewall filter add chain=forward dst-address=192.168.100.0/24 action=drop

# Allow VLAN77 to internet
/ip firewall filter add chain=forward src-address=192.168.77.0/24 out-interface=eth1 action=accept

# Default drop
/ip firewall filter add chain=forward action=drop
```

**VLAN100 (Management) Rules:**
```routeros
# Allow VLAN100 to all destinations (admin access)
/ip firewall filter add chain=forward src-address=192.168.100.0/24 action=accept

# Block all inbound to VLAN100 (no compromise path)
/ip firewall filter add chain=forward dst-address=192.168.100.0/24 action=drop
```

**WireGuard VPN Rules:**
```routeros
# Block WireGuard VPN to VLAN88 (home network)
/ip firewall filter add chain=forward src-address=10.8.0.0/24 dst-address=192.168.88.0/24 action=drop

# Block WireGuard VPN to VLAN100 (management)
/ip firewall filter add chain=forward src-address=10.8.0.0/24 dst-address=192.168.100.0/24 action=drop
```
...
/ip firewall nat add chain=dstnat protocol=udp dst-port=51820 to-addresses=192.168.77.174 to-ports=51820
```
...
| **VLAN hopping** | Properly configured switch (VLAN tagging), firewall rules |
| **Admin workstation compromise** | VLAN100 isolated (no inbound), VPN instead of direct access |
...
7. **K07 - Missing Network Segmentation**: VLANs, NetworkPolicy, firewall
...
1. **Network segmentation** (VLANs, firewall, VPN)
